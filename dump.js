...
var NSSearchPathForDirectoriesInDomains = getExportFunction(
  "f",
  "NSSearchPathForDirectoriesInDomains",
  "pointer",
  ["int", "int", "int"]
);
var wrapper_open = getExportFunction("f", "open", "int", [
  "pointer",
  "int",
  "int",
]);
...
function getTemporaryDir() {
  var tmpDir = ObjC.classes.NSTemporaryDirectory(); // 获取临时目录
  return tmpDir.toString();
}

function getAppBaseDir() {
  var mainBundle = ObjC.classes.NSBundle.mainBundle(); // 获取当前主Bundle
  return mainBundle.bundlePath().toString(); // 返回应用的主目录路径
}
...
function dumpModule(name) {
  ...
  var newmodpath = getAppBaseDir() + "/" + newmodname + ".fid";
...
function loadAllDynamicLibrary(app_path) {
  ...
  var filenames = defaultManager.contentsOfDirectoryAtPath_error_(
    app_path,
    errorPtr
  );
        ...
        if (file_name.hasSuffix_(".dylib")) {
          ...
          for (var j = 0; j < modules.length; j++) {
            if (modules[j].path.indexOf(file_name) != -1) {
              is_loaded = 1;
              console.log(
                "[frida-ios-dump]: " + file_name + " has been dlopen."
              );
              break;
            }
          }
          ...
          if (!is_loaded) {
            if (dlopen(allocStr(file_path.UTF8String()), 9)) {
              console.log(
                "[frida-ios-dump]: dlopen " + file_name + " success. "
              );
            } else {
              console.log(
                "[frida-ios-dump]: dlopen " + file_name + " failed. "
              );
            }
          }
        }
